...
const getPath = (input) => {
  ...
  for(let i = 0; i < input.length; i++) {
    const {x: pathX, y: pathY} = path[pathLength]
    let x = pathX
    let y = pathY
    const moveLength = Number(input[i].slice(1))
    pathLength+=moveLength;
    switch (input[i][0]) {
      case 'R': {
        for(let j = 1; j <= moveLength; j++){
          path.push({x: x + j, y})
        }
        //x+=moveLength
        break
  ...
  const {path, pathLength} = getPath(input1)
  const path2 = getPath(input2).path
    //console.log((i/pathLength) * 100)
    const crossings = lodash.intersectionWith(path2, path, (one, other) => console.log(one, other) || lodash.isEqual(one, other))
const r = lodash.map(crossings, ({x, y}) => x+y).sort(function(a, b){return a - b})
console.log(r, crossings)